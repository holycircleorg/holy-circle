...
router.get("/user/:memberId", (req, res) => {
...
// Edit badge
router.post("/edit", upload.single("icon"), (req, res) => {
...
// Alias: GET badges for member (frontend canonical)
router.get("/member/:memberId", (req, res) => {
  const memberId = Number(req.params.memberId);
  if (!memberId) return res.json({ badges: [] });
  ...
  db.all(
    ...
    (err, rows) => {
      if (err) {
        console.error("Error loading member badges:", err);
        return res.status(500).json({ badges: [] });
      }
